...
pub fn find_session(connection: &Connection, session_id: &str) -> Result<Session, AuthError> {
    connection
...
pub fn drop_session(connection: &Connection, session_id: &str) {
    let _: Option<()> = connection.del(&get_session_key(session_id)).ok();
}

pub fn store_session(connection: &Connection, session: &Session) -> Option<()> {
    connection
        .set_ex(
